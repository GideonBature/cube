...
impl Move {
    ...
    pub fn decode_sbe(bytes: &[u8]) -> Result<Move, MoveSBEDecodeError> {
        ...
        if bytes.is_empty() {
            return Err(
                MoveSBEDecodeError::MoveSBEInsufficientBytesForFromLengthPrefix { got_total: 0 },
            );
        }
        if bytes[0] != 0x00 {
        ...
        if bytes.len() < 5 {
            return Err(
                MoveSBEDecodeError::MoveSBEInsufficientBytesForFromLengthPrefix {
                    got_total: bytes.len(),
                },
            );
        }
        let from_len = u32::from_le_bytes(
            bytes[1..5]
                .try_into()
                .map_err(|_| MoveSBEDecodeError::MoveSBEFromLengthPrefixBytesConversionError)?,
        ) as usize;
        ...
        let from = RootAccount::decode_sbe(from_slice)
            .map_err(MoveSBEDecodeError::MoveSBEFromRootAccount)?;

        // 4 Decode the receiver (`to`) payload length prefix.
        if after_from.len() < 4 {
            return Err(
                MoveSBEDecodeError::MoveSBEInsufficientBytesForToLengthPrefix {
                    got_total: bytes.len(),
                },
            );
        }
        let to_len = u32::from_le_bytes(
            after_from[0..4]
                .try_into()
                .map_err(|_| MoveSBEDecodeError::MoveSBEToLengthPrefixBytesConversionError)?,
        ) as usize;
        ...
        let amount = u32::from_le_bytes(
            after_to[0..4]
                .try_into()
                .map_err(|_| MoveSBEDecodeError::MoveSBEAmountBytesConversionError)?,
        );
        ...
        let target =
            Target::decode_sbe(&after_to[4..12]).map_err(MoveSBEDecodeError::MoveSBETarget)?;
