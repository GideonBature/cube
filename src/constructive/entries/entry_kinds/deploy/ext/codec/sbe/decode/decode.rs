...
impl Deploy {
    ...
    pub fn decode_sbe(bytes: &[u8]) -> Result<Deploy, DeploySBEDecodeError> {
        if bytes.is_empty() {
            return Err(
                DeploySBEDecodeError::DeploySBEInsufficientBytesForRootAccountLengthPrefix {
                    got_total: 0,
                },
            );
        }
        ...
        let root_len = u32::from_le_bytes(bytes[1..5].try_into().map_err(|_| {
            DeploySBEDecodeError::DeploySBERootAccountLengthPrefixBytesConversionError
        })?) as usize;
        let after_root_len_prefix = &bytes[5..];
        if after_root_len_prefix.len() < root_len {
            return Err(
                DeploySBEDecodeError::DeploySBERootAccountLengthPrefixExceedsPayload {
                    root_len,
        ...
        if tail.len() < 4 {
            return Err(
                DeploySBEDecodeError::DeploySBEInsufficientBytesForProgramLengthPrefix {
                    got_total: bytes.len(),
                },
            );
        }
        let program_len =
            u32::from_le_bytes(tail[0..4].try_into().map_err(|_| {
                DeploySBEDecodeError::DeploySBEProgramLengthPrefixBytesConversionError
            })?) as usize;
        tail = &tail[4..];
        if tail.len() < program_len {
            return Err(
                DeploySBEDecodeError::DeploySBEProgramLengthPrefixExceedsPayload {
                    program_len,
        ...
        if tail.len() < 4 {
            return Err(
                DeploySBEDecodeError::DeploySBEInsufficientBytesForInitialBalance {
                    got_total: bytes.len(),
                },
            );
        }
        ...
        let target = Target::decode_sbe(&tail[0..8])
            .map_err(DeploySBEDecodeError::DeploySBETargetDecodeError)?;
        tail = &tail[8..];
